...
void showCsu(const Context *cp);
void showStructure(const Structure * pStructure) {
    
    const Context * cp = pStructure->GetContext();
    if (cp != NULL) {
        //showCsu(cp);
    }
    ...
    for (U32 i = 0; i < fieldCount; i++) {
        ...
        u32 offs = 0xffffffff;
        if (field.HasOffset()) offs = field.GetOffset();
        ...
        if (field.GetType().GetBaseType() == Type::BaseType_Structure) {
            printf("structure: %x\n", offs);
            const Type & t = field.GetType();
            const TypeStructure * ts = dynamic_cast<const TypeStructure *>(&t);
            if (ts != NULL) {
                showCsu( &(ts->GetStructure()) );
            }
            continue;
        }
...
void showCsu(const Context *cp) {
    enum Context::Type t = cp->GetType();
    printf("    %20s  %s\n", "type", (t==Context::Type_Class?"Class":
                                      t==Context::Type_Namespace?"Namespace":
                                      t==Context::Type_Struct?"Struct":
                                      t==Context::Type_Union?"Union":"unknown"));
    switch(t) {
    case Context::Type_Class:
    case Context::Type_Struct:
    {   const Structure *sp = dynamic_cast<const Structure *>(cp);
        if (cp == NULL) break; //impossible case
        showStructure(sp);
    }
        break;
...
void showContexts() {
    ssize_t s = GetContextCount();
    printf("CountextCount %ld \n", s);
    for (ssize_t i=0; i<s; i++) {
        const Context *cp = GetContext(i);
        printf("  Context %i\n", i);
        printf("    %20s  %s\n", "name",  cp->GetName());
        showCsu(cp);
    }
